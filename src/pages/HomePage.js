...
import { fetchBook, setQuery } from "../store/bookSlice";
...
const HomePage = () => {
  ...
  const { bookList, status, error, page, query } = useSelector(state => state.books);
  ...
  const onSubmit = (data) => {
    ...
    dispatch(fetchBook({ page: 1, limit: 10, query: data.searchQuery }));
  };

  const handleChangePage = (page) => {
    dispatch(fetchBook({ page: page, limit: 10, query }))
  };

  useEffect(() => {
    dispatch(fetchBook({ page: 1, limit: 10, query }))    
  }, [dispatch, query]);
